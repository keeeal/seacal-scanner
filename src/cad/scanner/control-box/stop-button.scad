...
module stop_button(pressed = false)
{
    ...
    color([ .7, .5, .1 ]) translate([ 0, 0, 37 ]) linear_extrude(21)
        difference()
    {
        rounded_square(72, r = 6.5, center = true);
        for (n = [0:3])
            rotate(90 * n)
                translate([ 59 / 2, 59 / 2 ]) circle(d = 8);
    }
    color([ .6, .6, .6 ]) translate([ 0, 0, 1 ])
        linear_extrude(55) rounded_square(69, r = 5, center = true);
